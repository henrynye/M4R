...
def get_player_nums(base_p_name):
    ...
    real_vals = matches_with_LR_pred['serve_win_rate']
    ...
    return real_vals, predicted_vals_mean, predicted_vals_median, predicted_vals_LR, predicted_vals_LR_2, predicted_vals_LR_3, predicted_vals_LR_4

def evaluate_preds(real_vals, predicted_vals, req
                   ):
    real_vals = np.array(real_vals)
    predicted_vals = np.array(predicted_vals)
    if len(real_vals) == 0 or len(predicted_vals) == 0:
        return None
    if len(real_vals) != len(predicted_vals):
        print(f"Length mismatch: {len(real_vals)} vs {len(predicted_vals)}")
        return None
    num_of_vals = len(real_vals)
    correct_threshold = 0.02
    def get_SD(preds):
    ...
    SD = get_SD(predicted_vals)
    MAE = get_MAE(predicted_vals)
    correct_rate = get_correct_rate(predicted_vals)
    absolute_baseline_errors = np.abs(np.subtract(real_vals, 0.635))


    return SD, MAE, correct_rate, absolute_baseline_errors
...
for player in tqdm(top_200_names):
    # edit_df(n)
    vals.append(get_player_nums(player))

total_matches = sum([len(v[0]) for v in vals])
print(f'Total matches: {total_matches}')

# Flatten vals so that each list contains all values for that prediction type across all players
# vals = [
#   [real_vals1, predicted_vals_mean1, predicted_vals_median1, ...],
#   [real_vals2, predicted_vals_mean2, predicted_vals_median2, ...],
#   ...
# ]
# We want:
# [*real_vals1, *real_vals2, ...], [*predicted_vals_mean1, *predicted_vals_mean2, ...], ...

flattened = [np.concatenate([v[i] for v in vals if v[i] is not None]) for i in range(7)]
real_vals_flat, predicted_vals_mean_flat, predicted_vals_median_flat, predicted_vals_LR_flat, predicted_vals_LR_2_flat, predicted_vals_LR_3_flat, predicted_vals_LR_4_flat = flattened

pred_names = ['mean', 'median', 'LR_1', 'LR_2', 'LR_3', 'LR_4']
for i, pred_name in enumerate(pred_names):
    SD, MAE, correct_rate, absolute_baseline_errors = evaluate_preds(real_vals_flat, flattened[i+1])
    print(f"Prediction: {pred_name}")
    print(f"SD: {SD:.4f}, MAE: {MAE:.4f}, Correct Rate: {correct_rate:.4f}")
    print(f"Absolute Baseline Errors: {np.mean(absolute_baseline_errors):.4f}")
